...
__global__ void reduce(float *input, float *output) {
    ...
    float *input_begin = input + blockIdx.x * blockDim.x; // 硬件与工具探索
    shared_input[threadIdx.x] = input_begin[threadIdx.x];
    __syncthreads();

    for (int i = blockDim.x / 2; i > 0; i /= 2) {
        if (threadIdx.x < i) { // 控制参与计算的线程
            shared_input[threadIdx.x] += shared_input[threadIdx.x + i];
        }
        __syncthreads();
    }
    if (threadIdx.x == 0) {
