...
__global__ void sgemm_kernel(float *A, float *B, float *C, const int M, const int N, const int K) {
    ...
    float a_reg_transpose[REG_NUM] = {0.0f};
    float sum[REG_NUM][REG_NUM] = {0.0f}; 

    // 加载A和B到共享内存
    for (int s = 0; s < K; s += K_NUM_PER_BLOCK) {
        for (int i = 0; i < REG_NUM; i++) {
            FETCH_FLOAT4(a_reg_transpose[0]) = FETCH_FLOAT4(A_tile_start[K * (ty * REG_NUM + i) + tx * REG_NUM + s]);
            shared_A[tx * REG_NUM][ty * REG_NUM + i] = a_reg_transpose[0];
            shared_A[tx * REG_NUM + 1][ty * REG_NUM + i] = a_reg_transpose[1];
            shared_A[tx * REG_NUM + 2][ty * REG_NUM + i] = a_reg_transpose[2]; 
            shared_A[tx * REG_NUM + 3][ty * REG_NUM + i] = a_reg_transpose[3];
        }
        ...
        for (int k = 0; k < K_NUM_PER_BLOCK; k++) {
            FETCH_FLOAT4(a_reg[0]) = FETCH_FLOAT4(shared_A[k][ty * REG_NUM]);
