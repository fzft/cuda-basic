...
__global__ void sgemm_kernel(float *A, float *B, float *C, const int M, const int N, const int K) {
    ...
    for (int s = 0; s < K; s += K_NUM_PER_BLOCK) {
        FETCH_FLOAT4(shared_A[ty][tx * NUM_PER_THREAD]) = FETCH_FLOAT4(A_tile_start[s + ty * K + tx * NUM_PER_THREAD]);
        FETCH_FLOAT4(shared_B[ty][tx * NUM_PER_THREAD]) = FETCH_FLOAT4(B_tile_start[N *(ty+s) + tx * NUM_PER_THREAD]);
        __syncthreads(); 

        // 外积 
        for (int k = 0; k < K_NUM_PER_BLOCK; k++) {
            a_reg[0] = shared_A[cty * 2][k]; 
            a_reg[1] = shared_A[cty * 2 + 1][k];
            b_reg[0] = shared_B[k][ctx * 2];
            b_reg[1] = shared_B[k][ctx * 2 + 1];
    ...
    for (int i = 0; i < REG_NUM; i++) {
        for (int j = 0; j < REG_NUM; j++) {
            C_ptr[N * (cty * REG_NUM + i) + (ctx * REG_NUM + j)] = sum[i][j];
        }
