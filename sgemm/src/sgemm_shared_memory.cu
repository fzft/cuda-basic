...
__global__ void sgemm_kernel(float *A, float *B, float *C, const int M, const int N, const int K) {
    ...
    __shared__ float shared_A[BLOCK_SIZE][K_];
    __shared__ float shared_B[K_][BLOCK_SIZE];

    for (int s = 0; s < K; s+= BLOCK_SIZE) {
        shared_A[threadIdx.y][threadIdx.x + s] = A_row_start[threadIdx.y * K + s + threadIdx.x];
        shared_B[threadIdx.y + s][threadIdx.x] = B_col_start[(threadIdx.y + s) * N + threadIdx.x];
    }
...
int main() {
    ...
    sgemm_kernel<threads_per_block, k><<<grid, block>>>(d_A, d_B, d_C, m, n, k);
